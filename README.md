...
There are multiple ways to prepare the project for development. Described here is a method to run the auxiliary services using docker containers and a method to run these services directly using system packages. For both paths you should go through thhe common dependencies and setting up the python environment first.
...
# Run server (IP defaults to 127.0.0.1 and port defaults to 8000)
# (Requires that the database is up and running)
