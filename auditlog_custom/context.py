...
@contextlib.contextmanager
def safe_set_extra_data(context_data):
    return call_context_manager(context_data)


def call_context_manager(context_data):
    """Connect a signal receiver with current user attached."""
    print(f"CUSTOM CONTEXT DATA CALLED WITH", context_data)
...
def _set_actor(auditlog, instance, sender):
    print("SETTING ACTOR", auditlog, ",", instance, ",", sender)
    LogEntry = get_logentry_model()
    auth_user_model = get_user_model()
    if "actor" in auditlog:
        actor = auditlog.get("actor")
        print("ACTOR IS", actor)
        if sender == LogEntry and isinstance(actor, auth_user_model) and instance.actor is None:
            instance.actor = actor
            instance.actor_email = "omitted"
