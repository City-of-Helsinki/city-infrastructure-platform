...
def get_mountreal_to_mountplan_mapping(max_distance: float):
    ...
    for mr in mr_qset:
        ...
        if not plan_instances.exists():
            not_found_for_mrs.append(mr.id)
        else:
            mr_to_plan_instance[mr.id] = plan_instances.values_list("id", flat=True)
...
def get_additionalsignreal_to_additionalsignplan_mapping(max_distance: float):
    ...
    for adsr in adsr_qset:
        ...
        if not plan_instances.exists():
            not_found_for_adsr.append(adsr.id)
        else:
            adsr_to_plan_instance[adsr.id] = plan_instances.values_list("id", flat=True)
...
def get_trafficsignreal_to_trafficsignplan_mapping(max_distance: float):
    ...
    for tsr in tsr_qset:
        ...
        if not plan_instances.exists():
            not_found_for_tsr.append(tsr.id)
        else:
            tsr_to_plan[tsr.id] = plan_instances.values_list("id", flat=True)
    ...
    for k, v in real_to_plan_instance_mapping_dict.items():
        for p_id in v:
            if p_id in unique_ids:
                ...
                unique_ids.add(p_id)
    return unique_ids - duplicate_ids, duplicate_ids
...
def find_and_update_plan_instances_to_reals(
    ...
    reals_to_plan_instances, not_found_for_reals = _get_real_to_plan_instance_mapping(real_model, max_distance)
    if do_db_update:
        unique_ids, duplicate_ids = get_duplicate_plan_instance_ids(reals_to_plan_instances)
    else:
        unique_ids = set()
        duplicate_ids = set()
    print("JF uniques duplicates", len(unique_ids), len(duplicate_ids))
    return update_plan_instances_to_reals(
...
def update_plan_instances_to_reals(
    ...
    Return skipped real object ids with reason.
    """
...
def _get_best_matching_plan_instance(real_id, real_model, plan_instance_ids, plan_model):
    ...
    if len(plan_instances) == 0:
        print(f"skip real: {real_id}, no unique plans")
        return None
    ...
    if len(plan_instances) == 1:
        ...
        if len(same_distances) == 1:
            ...
            print("same distances resolve for rid", real_id)
...
def _get_same_distance_plan_instance(same_distances):
    ...
    NOTE: in the future there will be decision_date also on plan instances.
...
def _get_csv_headers(real_model):
    ...
    if real_model is TrafficSignReal:
        base_headers.extend(["mount_real", "pi_mount_plan"])
    elif real_model is AdditionalSignReal:
        base_headers.extend(["parent", "additional_information"])
...
def _rows_for_results_csv(results, real_model):
    ...
    for rid, pi in results.items():
        ...
        if real_model is TrafficSignReal:
            base_row.extend([str(real_obj.device_type.code, real_obj.mount_real_id), str(pi.mount_plan)])
        elif real_model is AdditionalSignReal:
