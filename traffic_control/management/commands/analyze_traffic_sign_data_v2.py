...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: dict) -> None:
        ...
        if not os.path.exists(previous_mount_file):
            self.stderr.write(self.style.ERROR(f"Previous mount file not found: {previous_mount_file}"))
            return None

        previous_sign_file = options["previous_sign_file"]
        if not os.path.exists(previous_sign_file):
            self.stderr.write(self.style.ERROR(f"Previous sign file not found: {previous_sign_file}"))
            return None
        ...
        for report in reports:
            ...
            # Collect invalid device type codes for printing
            if report_type == "INVALID DEVICE TYPE CODES":
        ...
        # Print invalid device type codes if any were found
        if invalid_codes:
