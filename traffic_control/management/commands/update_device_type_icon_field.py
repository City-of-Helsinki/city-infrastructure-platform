        ...
        with set_actor(get_system_user()):
            ...
            for device_type in TrafficControlDeviceType.objects.all():
                if not device_type.icon:
                    self.stdout.write(
                        f"Skipped TrafficControlDeviceType without icon string. Device type: {device_type}"
                    )
                    continue
                ...
                try:
                    ...
                    device_type.icon_file = TrafficControlDeviceTypeIcon.objects.get(
                        file__endswith=f"/{device_type.icon}"
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Updated {len(updated_device_types)} TrafficControlDeviceType objects.")
            )
