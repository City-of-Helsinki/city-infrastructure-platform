...
def _mock_importer_instance() -> MagicMock:
    """Return a MagicMock importer instance with required attributes pre-configured.

    Returns:
        MagicMock: Mock importer with _preprocessing_duration_s set to a float.
    """
    instance = MagicMock()
    instance.run.return_value = _MOCK_SUMMARY
    instance._preprocessing_duration_s = 0.0
    return instance
...
def test_default_object_types_and_phases(tmp_path: Path) -> None:
    ...
    with patch(_IMPORTER_PATH) as mock_cls:
        mock_instance = _mock_importer_instance()
...
def test_dry_run_true_when_flag_set(tmp_path: Path) -> None:
    ...
    with patch(_IMPORTER_PATH) as mock_cls:
        ...
        mock_instance._preprocessing_duration_s = 0.0
