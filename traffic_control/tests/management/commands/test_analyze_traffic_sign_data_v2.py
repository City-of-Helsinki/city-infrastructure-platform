...
def test_traffic_sign_analyzer_v2_reports():
    ...
    assert "CHANGED RECORDS" in report_types
    assert "UNCHANGED RECORDS" in report_types
    ...
    assert "INVALID DEVICE TYPE CODES" in report_types
    assert "MISSING MOUNTS FROM DATABASE" in report_types
    assert "MISSING TRAFFIC SIGNS FROM DATABASE" in report_types
    assert "MISSING ADDITIONAL SIGNS FROM DATABASE" in report_types
    assert "DUPLICATE SIGNS ON SAME MOUNT" in report_types
    assert "DUPLICATE SIGNS ON SAME MOUNT (EXACT CODE)" in report_types
    assert "ADDED DOUBLE SIDED ZEBRA CROSSINGS" in report_types
    assert "MOUNTS FOUND IN DATABASE" in report_types
    assert "TRAFFIC SIGNS FOUND IN DATABASE" in report_types
    assert "ADDITIONAL SIGNS FOUND IN DATABASE" in report_types
    assert "MAIN SIGNS WITH PARENT" in report_types
    assert "MOUNTS WITH REMOVED SIGNS" in report_types
...
def test_traffic_sign_analyzer_v2_duplicate_detection():
    ...
    assert "mount_location" in duplicate
    assert "duplicate_signs" in duplicate
    assert isinstance(duplicate["duplicate_signs"], list)
    assert len(duplicate["duplicate_signs"]) >= 2
