...
class TestReactivateSelectedUsersAction:
    ...
    @pytest.mark.django_db
    @pytest.mark.parametrize("was_active", [True, False], ids=["was_active", "was_inactive"])
    ...
    def test_save_model_affects_deactivation_status(self, rf, admin_user, was_active, is_active, is_superuser):
        target_user = UserFactory.create(is_active=was_active)
        if not was_active:
            UserDeactivationStatus.objects.create(user=target_user, deactivated_at=timezone.now() - timedelta(days=5))
        assert UserDeactivationStatus.objects.filter(user=target_user).exists() is not was_active  # Sanity check
        ...
        is_active_flipped = is_active != was_active
        ...
        if is_operation_permitted:
            ...
            assert target_user.is_active == was_active, "The user update rejected, should have old is_active value."
            assert (
        ...
        if admin_user.has_activation_status_change_permission():
            ...
            should_deactivation_status_exist = not was_active
